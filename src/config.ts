...
const DEFAULT_CONFIG: AICodeContextConfig = {
  ...
  includePatterns: ['**/*.js', '**/*.ts', '**/*.tsx', '**/*.jsx', '**/*.py', '**/*.java', '**/*.cpp', '**/*.c', '**/*.h'],
...
export class ConfigManager {
  ...
  getApiKey(): string {
    return this.config.apiKey || process.env.OPENAI_API_KEY || process.env.ANTHROPIC_API_KEY || '';
  }
  ...
  async validateConfig(): Promise<{ valid: boolean; errors: string[] }> {
    ...
    if (!this.getApiKey()) {
      errors.push('API key is required. Set it in .aicontext.json or as environment variable (OPENAI_API_KEY or ANTHROPIC_API_KEY)');
    }
