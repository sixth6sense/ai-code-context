...
Please analyze the following code and provide structured insights.`;
  ...
  private buildDiffAnalysisPrompt(
    projectContext: ProjectContext,
    language: string
  ): string {
    return `You are analyzing code changes in a ${
      projectContext.type
...
Focus on helping developers understand the change quickly and effectively.`;
  ...
  private extractSections(response: string): any {
    ...
    const suggestionsMatch = response.match(
      /(?:Suggestions|SUGGESTIONS)[:\-\s]*([\s\S]*?)$/i
    );
    if (suggestionsMatch) {
